...
function CompetitiveLandscapeMap({ slide }: LayoutProps) {
  ...
  return <div className="scene-wrap"><SlideHeading slide={slide} /><div className="landscape-map"><div className="axis-x">Mayor personalizacion</div><div className="axis-y">Mayor facilidad docente</div><div className="grid-markers" />{points.map((p)=><span key={p.label} className="market-point" style={{left:`${p.x}%`,top:`${p.y}%`}}>{p.label}</span>)}<div className="opportunity-zone">Opportunity Space - eduTechIA</div></div>{renderListItems(slide.items)}</div>;
}
